...
static void insert(struct client *head, struct client *node);
static void insert(struct client *head, struct client *node);
static struct client* find(int socketfd, struct client *head);  
static struct client* delete(struct client *head, struct client *deletenode);
static void retransmission(struct client *head,struct client **mainhead);
static int getpackettype(char *message);
static int formdatapacket(char *packet, char *filename, int file_position, int block_num);
static int formerrorpacket(char *packet,int error_code,char *message);
...
void retransmission(struct client *head,struct client **mainhead)
{
	...
	else
	{
		...
		else if (seconds > 0.1 && head->end_of_file == 1)
		{
			...
			//int file_position = formdatapacket(packet,head->filename,new_position,head->block_num); //ununsed
						...
						else
						{
							...
							{	
								...
								if (temp->lost_ack >= 4)
								{
									...
									//int file_position = formdatapacket(packet,temp->filename,new_position,temp->block_num); //unused
