...
PDU_LIST_st *snmp_walk(struct  snmp_session *pSession, oid *rootOID, int rootOID_len, int reps)
{
...
int iporting_net_snmp_walk(char *host, char *community, char *oid_name, int reps, PDU_LIST_st **pdu_list)
{
    size_t anOID_len = MAX_OID_LEN;
    oid anOID[MAX_OID_LEN];
    struct snmp_session ss, *sp;
    
    if(host == NULL || community == NULL || oid_name == NULL || reps <= 0 || pdu_list == NULL)
    {
        printf("%s %d:input wrong\n", __FUNCTION__, __LINE__);
        return-1;
    }
    ...
    if(snmp_parse_oid("sysname", anOID, &anOID_len) == 0)
    {
        printf("%s %d:can not find oid:%s\n", __FUNCTION__, __LINE__, oid_name);
        snmp_close(sp);
        return -1;
    }
    if((*pdu_list = snmp_walk(sp, anOID, anOID_len, reps)) == NULL)
    {
        snmp_close(sp);
        return -1;
    }
...
void synchronous (void)
{
  ...
  for (hp = hosts; hp->name; hp++) {
    ...
    if(iporting_net_snmp_walk(host, community, oid_name, reps, &pdu_list) != 0)
    {
