...
int snmp_parse_pdulist(PDU_LIST_st *pPDUList, char *oid_name, SnmpTableFun fun, int DataLen, SNMP_DATA_LIST_st **pDataList);
...
int snmp_parse_pdulist(PDU_LIST_st *pPDUList, char *oid_name, SnmpTableFun fun, int DataLen, SNMP_DATA_LIST_st **pDataList)
{
    ...
    if(pPDUList == NULL || pPDUList->response == NULL || oid_name == NULL || *oid_name == 0 || fun == NULL || DataLen <= 0 || pDataList == NULL)
    {
        istc_log("input wrong\n");
...
int istc_snmp_parse_data(char *oid_name, SnmpTableFun fun, int DataLen, SNMP_DATA_LIST_st **pDataList)
{
    ...
    if(snmp_parse_pdulist(pdu_list, oid_name, fun, DataLen, pDataList) != 0)
    {
