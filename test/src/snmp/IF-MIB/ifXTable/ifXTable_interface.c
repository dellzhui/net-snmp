...
static void _ifXTable_container_init(
    ifXTable_interface_ctx *if_ctx);
static void _ifXTable_container_shutdown(
    ifXTable_interface_ctx *if_ctx);
 ...
 */
void
_ifXTable_initialize_interface(ifXTable_registration * reg_ptr,  u_long flags)
{
    netsnmp_baby_steps_access_methods *access_multiplexer =
    ...
    DEBUGMSGTL(("internal:ifXTable:_ifXTable_initialize_interface","called\n"));
    ...
    reginfo = netsnmp_handler_registration_create("ifXTable", handler,
                                                  ifXTable_oid,
                                                  ifXTable_oid_size,
...
rc = ifLinkUpDownTrapEnable_get(rowreq_ctx, (u_long *)var->val.string );
        break;
...
rc = ifCounterDiscontinuityTime_get(rowreq_ctx, (u_long *)var->val.string );
        break;
     ...
     default:
        if (IFXTABLE_MIN_COL <= column && column <= IFXTABLE_MAX_COL) {
            ...
            snmp_log(LOG_ERR,
                "unknown column %d in _ifXTable_get_column\n", column);
        }
        break;
    }
                         ...
                         netsnmp_variable_list *var, int column )
{
    ...
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifName",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifName_check_value( rowreq_ctx, (char *)var->val.string, var->val_len );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifName_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER,
        sizeof( rowreq_ctx->data.ifInMulticastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifInMulticastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifInMulticastPkts_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifInMulticastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFINBROADCASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER,
        sizeof( rowreq_ctx->data.ifInBroadcastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifInBroadcastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifInBroadcastPkts_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifInBroadcastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFOUTMULTICASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER,
        sizeof( rowreq_ctx->data.ifOutMulticastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifOutMulticastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifOutMulticastPkts_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifOutMulticastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFOUTBROADCASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER,
        sizeof( rowreq_ctx->data.ifOutBroadcastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifOutBroadcastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifOutBroadcastPkts_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifOutBroadcastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCInOctets ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCInOctets",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCInOctets_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCInOctets_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCINUCASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCInUcastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCInUcastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCInUcastPkts_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCInUcastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCINMULTICASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCInMulticastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCInMulticastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCInMulticastPkts_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCInMulticastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCINBROADCASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCInBroadcastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCInBroadcastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCInBroadcastPkts_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCInBroadcastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCOUTOCTETS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCOutOctets ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCOutOctets",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCOutOctets_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCOutOctets_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCOUTUCASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCOutUcastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCOutUcastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCOutUcastPkts_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCOutUcastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCOUTMULTICASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCOutMulticastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCOutMulticastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCOutMulticastPkts_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCOutMulticastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFHCOUTBROADCASTPKTS:
    rc = netsnmp_check_vb_type_and_size( var, ASN_COUNTER64,
        sizeof( rowreq_ctx->data.ifHCOutBroadcastPkts ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHCOutBroadcastPkts",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHCOutBroadcastPkts_check_value( rowreq_ctx, *((U64 *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHCOutBroadcastPkts_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
 ...
 && ( *var->val.integer != IFLINKUPDOWNTRAPENABLE_DISABLED )
    ...
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifLinkUpDownTrapEnable",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifLinkUpDownTrapEnable_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifLinkUpDownTrapEnable_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    rc = netsnmp_check_vb_type_and_size( var, ASN_GAUGE,
        sizeof( rowreq_ctx->data.ifHighSpeed ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifHighSpeed",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifHighSpeed_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifHighSpeed_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
 ...
 && ( *var->val.integer != TRUTHVALUE_FALSE )
    ...
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifPromiscuousMode",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifPromiscuousMode_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifPromiscuousMode_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    case COLUMN_IFCONNECTORPRESENT:
    rc = netsnmp_check_vb_type_and_size( var, ASN_INTEGER,
        sizeof( rowreq_ctx->data.ifConnectorPresent ) );
    /* check that the value is one of defined enums */
    if( (SNMPERR_SUCCESS == rc)
 && ( *var->val.integer != TRUTHVALUE_TRUE )
 && ( *var->val.integer != TRUTHVALUE_FALSE )
    ...
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifConnectorPresent",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifConnectorPresent_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifConnectorPresent_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
    ...
    if(SNMPERR_SUCCESS != rc) {
        ...
        rc = ifAlias_check_value( rowreq_ctx, (char *)var->val.string, var->val_len );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
    ...
    rc = netsnmp_check_vb_type_and_size( var, ASN_TIMETICKS,
        sizeof( rowreq_ctx->data.ifCounterDiscontinuityTime ) );
    if(SNMPERR_SUCCESS != rc) {
        DEBUGMSGTL(("ifXTable:_ifXTable_check_column:ifCounterDiscontinuityTime",
                    "varbind validation failed (eg bad type or size)\n"));
    }
    else {
        rc = ifCounterDiscontinuityTime_check_value( rowreq_ctx, *((u_long *)var->val.string) );
        if((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) &&
           (MFD_NOT_VALID_NOW != rc)) {
            snmp_log(LOG_ERR, "bad rc %d from ifCounterDiscontinuityTime_check_value\n", rc);
            rc = SNMP_ERR_GENERR;
        }
    }
        break;
                         ...
                         netsnmp_request_info *requests)
{
    ...
    for(;requests; requests = requests->next) {
        ...
        rc = _ifXTable_check_column(rowreq_ctx, requests->requestvb, tri->colnum);
        if(rc) {
            netsnmp_request_set_error(requests, SNMP_VALIDATE_ERR(rc));
            break;
        }
...
_ifXTable_undo_setup_column( ifXTable_rowreq_ctx *rowreq_ctx, int column )
{
    ...
    case COLUMN_IFHIGHSPEED:
        ...
        rc = ifHighSpeed_undo_setup(rowreq_ctx );
        break;
    ...
    case COLUMN_IFCONNECTORPRESENT:
        ...
        rc = ifConnectorPresent_undo_setup(rowreq_ctx );
        break;
    ...
    case COLUMN_IFCOUNTERDISCONTINUITYTIME:
        ...
        rc = ifCounterDiscontinuityTime_undo_setup(rowreq_ctx );
        break;

     default:
         snmp_log(LOG_ERR,"unknown column %d in _ifXTable_undo_setup_column\n", column);
         break;
    }
                       ...
                       netsnmp_variable_list *var, int column )
{
    ...
    case COLUMN_IFNAME:
        rowreq_ctx->column_set_flags |= COLUMN_IFNAME_FLAG;
        rc = ifName_set(rowreq_ctx, (char *)var->val.string, var->val_len );
        break;

    /* ifInMulticastPkts(2)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFINMULTICASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFINMULTICASTPKTS_FLAG;
        rc = ifInMulticastPkts_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;

    /* ifInBroadcastPkts(3)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFINBROADCASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFINBROADCASTPKTS_FLAG;
        rc = ifInBroadcastPkts_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;

    /* ifOutMulticastPkts(4)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFOUTMULTICASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFOUTMULTICASTPKTS_FLAG;
        rc = ifOutMulticastPkts_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;

    /* ifOutBroadcastPkts(5)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFOUTBROADCASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFOUTBROADCASTPKTS_FLAG;
        rc = ifOutBroadcastPkts_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;

    /* ifHCInOctets(6)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINOCTETS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCINOCTETS_FLAG;
        rc = ifHCInOctets_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCInUcastPkts(7)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINUCASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCINUCASTPKTS_FLAG;
        rc = ifHCInUcastPkts_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCInMulticastPkts(8)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINMULTICASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCINMULTICASTPKTS_FLAG;
        rc = ifHCInMulticastPkts_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCInBroadcastPkts(9)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINBROADCASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCINBROADCASTPKTS_FLAG;
        rc = ifHCInBroadcastPkts_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCOutOctets(10)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTOCTETS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCOUTOCTETS_FLAG;
        rc = ifHCOutOctets_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCOutUcastPkts(11)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTUCASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCOUTUCASTPKTS_FLAG;
        rc = ifHCOutUcastPkts_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCOutMulticastPkts(12)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTMULTICASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCOUTMULTICASTPKTS_FLAG;
        rc = ifHCOutMulticastPkts_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifHCOutBroadcastPkts(13)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTBROADCASTPKTS:
        rowreq_ctx->column_set_flags |= COLUMN_IFHCOUTBROADCASTPKTS_FLAG;
        rc = ifHCOutBroadcastPkts_set(rowreq_ctx, *((U64 *)var->val.string) );
        break;

    /* ifLinkUpDownTrapEnable(14)/INTEGER/ASN_INTEGER/long(u_long)//l/A/W/E/r/d/h */
    case COLUMN_IFLINKUPDOWNTRAPENABLE:
        rowreq_ctx->column_set_flags |= COLUMN_IFLINKUPDOWNTRAPENABLE_FLAG;
        rc = ifLinkUpDownTrapEnable_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;

    /* ifHighSpeed(15)/GAUGE/ASN_GAUGE/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFHIGHSPEED:
        rowreq_ctx->column_set_flags |= COLUMN_IFHIGHSPEED_FLAG;
        rc = ifHighSpeed_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;
    ...
    case COLUMN_IFCONNECTORPRESENT:
        rowreq_ctx->column_set_flags |= COLUMN_IFCONNECTORPRESENT_FLAG;
        rc = ifConnectorPresent_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;
    ...
    case COLUMN_IFCOUNTERDISCONTINUITYTIME:
        rowreq_ctx->column_set_flags |= COLUMN_IFCOUNTERDISCONTINUITYTIME_FLAG;
        rc = ifCounterDiscontinuityTime_set(rowreq_ctx, *((u_long *)var->val.string) );
        break;
                         ...
                         netsnmp_request_info *requests)
{
    ...
    if (rowreq_ctx->rowreq_flags & MFD_ROW_DIRTY) {
        snmp_log(LOG_WARNING, "ifXTable row dirty flag still set after undo_commit\n");
                       ...
                       netsnmp_variable_list *var, int column )
{
    ...
    /* ifName(1)/DisplayString/ASN_OCTET_STR/char(char)//L/A/W/e/R/d/H */
    case COLUMN_IFNAME:
        rc = ifName_undo(rowreq_ctx);
        break;

    /* ifInMulticastPkts(2)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFINMULTICASTPKTS:
        rc = ifInMulticastPkts_undo(rowreq_ctx);
        break;

    /* ifInBroadcastPkts(3)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFINBROADCASTPKTS:
        rc = ifInBroadcastPkts_undo(rowreq_ctx);
        break;

    /* ifOutMulticastPkts(4)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFOUTMULTICASTPKTS:
        rc = ifOutMulticastPkts_undo(rowreq_ctx);
        break;

    /* ifOutBroadcastPkts(5)/COUNTER/ASN_COUNTER/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFOUTBROADCASTPKTS:
        rc = ifOutBroadcastPkts_undo(rowreq_ctx);
        break;

    /* ifHCInOctets(6)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINOCTETS:
        rc = ifHCInOctets_undo(rowreq_ctx);
        break;

    /* ifHCInUcastPkts(7)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINUCASTPKTS:
        rc = ifHCInUcastPkts_undo(rowreq_ctx);
        break;

    /* ifHCInMulticastPkts(8)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINMULTICASTPKTS:
        rc = ifHCInMulticastPkts_undo(rowreq_ctx);
        break;

    /* ifHCInBroadcastPkts(9)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCINBROADCASTPKTS:
        rc = ifHCInBroadcastPkts_undo(rowreq_ctx);
        break;

    /* ifHCOutOctets(10)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTOCTETS:
        rc = ifHCOutOctets_undo(rowreq_ctx);
        break;

    /* ifHCOutUcastPkts(11)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTUCASTPKTS:
        rc = ifHCOutUcastPkts_undo(rowreq_ctx);
        break;

    /* ifHCOutMulticastPkts(12)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTMULTICASTPKTS:
        rc = ifHCOutMulticastPkts_undo(rowreq_ctx);
        break;

    /* ifHCOutBroadcastPkts(13)/COUNTER64/ASN_COUNTER64/U64(U64)//l/A/W/e/r/d/h */
    case COLUMN_IFHCOUTBROADCASTPKTS:
        rc = ifHCOutBroadcastPkts_undo(rowreq_ctx);
        break;
    ...
    /* ifHighSpeed(15)/GAUGE/ASN_GAUGE/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFHIGHSPEED:
        rc = ifHighSpeed_undo(rowreq_ctx);
        break;
    ...
    /* ifConnectorPresent(17)/TruthValue/ASN_INTEGER/long(u_long)//l/A/W/E/r/d/h */
    case COLUMN_IFCONNECTORPRESENT:
        rc = ifConnectorPresent_undo(rowreq_ctx);
        break;
    ...
    /* ifCounterDiscontinuityTime(19)/TimeStamp/ASN_TIMETICKS/u_long(u_long)//l/A/W/e/r/d/h */
    case COLUMN_IFCOUNTERDISCONTINUITYTIME:
        rc = ifCounterDiscontinuityTime_undo(rowreq_ctx);
        break;
                         ...
                         netsnmp_request_info *requests)
{
    ...
    DEBUGMSGTL(("internal:ifXTable:_mfd_ifXTable_irreversible:commit","called\n"));
